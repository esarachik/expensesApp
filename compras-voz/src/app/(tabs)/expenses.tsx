...
import { getAllTransactions, deleteTransaction, getMonthlySummary } from '@/services/database';
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>      
      <View style={[styles.summaryCard, { backgroundColor: colorScheme === 'dark' ? '#1e1e1e' : '#f5f5f5' }]}>
        <Text style={[styles.summaryTitle, { color: colors.text }]}>Resumen del mes</Text>
        <View style={styles.summaryRow}>
          <View style={styles.summaryItem}>
            <Text style={[styles.summaryLabel, { color: colors.icon }]}>Ingresos</Text>
            <Text style={[styles.summaryValue, { color: '#4CAF50' }]}>
              ${summary.totalIngresos.toLocaleString()}
            </Text>
          </View>
          <View style={styles.summaryItem}>
            <Text style={[styles.summaryLabel, { color: colors.icon }]}>Egresos</Text>
            <Text style={[styles.summaryValue, { color: '#F44336' }]}>
              ${summary.totalEgresos.toLocaleString()}
            </Text>
          </View>
          <View style={styles.summaryItem}>
            <Text style={[styles.summaryLabel, { color: colors.icon }]}>Balance</Text>
            <Text
              style={[
                styles.summaryValue,
      ...
      <View style={[styles.tableHeader, { borderBottomColor: colorScheme === 'dark' ? '#333' : '#e0e0e0' }]}>
      ...
      {transactions.length === 0 ? (
        ...
        <FlatList
          ...
          ItemSeparatorComponent={() => (
            <View style={{ height: 1, backgroundColor: colorScheme === 'dark' ? '#333' : '#eee' }} />
          )}
        />
      )}
    </View>
